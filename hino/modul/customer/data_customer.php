<div class="page-content-wrapper">
		<div class="page-content">
			...
			<!-- BEGIN SAMPLE FORM PORTLET-->   
					 ...
					 <form method="post" action="">
                        <div class="row">
							...
							<div class="col-md-2">
								...
								<select required class="form-control form-control-inline select2me" name="sales">
             ...
             <!-- END SAMPLE FORM PORTLET-->
             <?php } elseif (($_SESSION[level] == 'admin' OR $_SESSION[level] == 'manager') ) { ?>
             <!-- BEGIN SAMPLE FORM PORTLET-->   
               <div class="portlet box red">
                  <div class="portlet-title">
                  <div class="caption">
			...
			$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan, a.nama_pengurus, a.salesman,
								   ...
								   LEFT JOIN (SELECT data_import,from_salesman,to_salesman FROM tb_import 
								   			  WHERE to_salesman = '$sales' AND deleted_flag = 0)  d ON d.data_import = a.id_pelanggan
								   ...
								   AND (b.tgl_aktivitas BETWEEN '$from' AND '$to' )
					 ...
					 <form method="post" action="">
                        <div class="row">
							...
							<div class="col-md-2">
								<label class="control-label">Sales / SPV</label>		
								<select required class="form-control form-control-inline select2me" name="sales">
										<option value="" selected="selected">- Pilih Sales / SPV -</option>
											<?php
 											$sql= mysql_query("SELECT * FROM ms_users WHERE username IN ($inUsers) AND level IN ('supervisor','sales') AND blokir = 'N' ORDER BY username ASC");
 											while($row=mysql_fetch_array($sql))
																	{
				...
				<div class="col-md-12">
					...
					<div class="portlet box purple">
						...
						<div class="portlet-body">
							...
							<?php
								if($_SESSION[sublevel] == 'unit'){
									$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan,
																  a.sesuai_ktp, 
																  DATE_FORMAT(b.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas, 
																  DATE_FORMAT(b.tgl_kunjungan_berikut, '%d %M %Y') AS 
																  tgl_kunjungan_berikut,k.status_prospek,
																  d.from_salesman, d.to_salesman, e.nama_database 
								   						   FROM tb_pelanggan a
								   						   LEFT JOIN v_unit_latest_activity_id i ON i.id_pelanggan = a.id_pelanggan
								   						   LEFT JOIN tb_aktivitas_harian_unit b ON b.id_aktivitas_harian = i.max_aktivitas
									...
									$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan, 
																  a.nama_pengurus, a.jabatan, a.salespart,
																  a.sesuai_ktp,  
																  DATE_FORMAT(b.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas, 
																  DATE_FORMAT(b.tgl_kunjungan_berikut, '%d %M %Y') AS 
																  tgl_kunjungan_berikut, 
																  d.from_salesman, d.to_salesman, e.nama_database 
														   FROM tb_pelanggan a
														   LEFT JOIN v_part_latest_activity_id i ON i.id_pelanggan = a.id_pelanggan
														   LEFT JOIN tb_aktivitas_harian_part b ON b.id_aktivitas_harian = i.max_aktivitas
														   ...
														   WHERE a.salespart = '$_SESSION[username]'
														   AND a.deleted_flag = '0'
														   AND (
														   		 	d.to_salesman = '$_SESSION[username]'
																AND d.deleted_flag = '0'
									...
									$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan,
														   ...
														   WHERE a.salesserv = '$_SESSION[username]'
														   AND a.deleted_flag = '0'
														   AND (
														   		 	d.to_salesman = '$_SESSION[username]'
																AND d.deleted_flag = '0'
						...
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								<?php
							 	...
							 	?>
								<tr>
									<td><?php echo str_replace($en, $id, $row['tgl_prospek']); ?></td>
