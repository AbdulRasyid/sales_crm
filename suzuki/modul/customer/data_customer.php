<div class="page-content-wrapper">
		<div class="page-content">
			...
			<!-- BEGIN SAMPLE FORM PORTLET-->   
               <div class="portlet box red">
                  <div class="portlet-title">
				 	...
				 	<div class="actions">
				 		<?php if($_SESSION[level] == 'sales' OR $_SESSION[level] == 'supervisor') { ?>
						<a href="?module=data_customer&act=tambah" class="btn btn-default btn-sm">
						<i class="fa fa-plus"></i> Tambah Customer </a>
						<?php } ?>
						<?php if($_SESSION[level] == 'sales' AND $_SESSION[sublevel] == 'part') { ?>
						<a href="javascript:;" class="btn btn-default btn-sm">
						<i class="fa fa-user"></i> Tambah Aktivitas Harian </a>
						<?php } ?>
				  	</div>
                  </div>
					 ...
					 <form method="post" action="">
                        <div class="row">
							...
							<div class="col-md-2">
								...
								<?php } ?>		
								<input class="form-control form-control-inline "  type="text" readonly name="sales" id="sales" value="<?php echo $_SESSION[username] ; ?>" />
							</div>

							<div class="col-md-2">
								<label class="control-label">Nama Customer</label>		
								<input class="form-control form-control-inline "  type="text"  name="nama_perush" 
								id="alamat" placeholder="Nama Customer"  />
							</div>

							<div class="col-md-2">
								<label class="control-label">Alamat</label>		
								<input class="form-control form-control-inline "  type="text"  name="alamat" 
								id="alamat" placeholder="Input Alamat"  />
							</div>
             ...
             <?php } elseif ($_SESSION[level] == 'supervisor' OR $_SESSION[level] == 'admin') { ?>
             <!-- BEGIN SAMPLE FORM PORTLET-->   
               <div class="portlet box red">
                  <div class="portlet-title">
                  <div class="caption">
                  ...
                  <div class="portlet-body">
					 ...
					 <form method="post" action="">
                        <div class="row">
							...
							<div class="col-md-2">
								<label class="control-label">Sales / SPV</label>		
								<select class="form-control form-control-inline select2me" name="sales" required>
										<option selected="selected" value="" >- Pilih Sales / SPV -</option>
											<?php
 											$sql= mysql_query("SELECT * FROM ms_users WHERE id_parent = '$_SESSION[id]' AND level = 'sales' AND blokir = 'N' ORDER BY username ASC");
							...
							<div class="col-md-2">
								<label class="control-label">Alamat</label>		
								<input class="form-control form-control-inline "  type="text"  name="alamat" 
								autocomplete="off"  placeholder="Input Alamat" />
							</div>
                        ...
                        <div class="row">
							...
							<div class="col-md-2">
								...
								<select class="form-control form-control-inline select2me" name="sales"  required >
										<option  selected="selected" value="">- Pilih Sales / SPV -</option>
											<?php
 											$sql= mysql_query("SELECT * FROM ms_users WHERE divisi = '$_SESSION[divisi]' AND level IN ('supervisor','sales') AND sublevel = '$_SESSION[sublevel]'  AND blokir = 'N' ORDER BY username ASC");
 											while($row=mysql_fetch_array($sql))
																	{
											$id = $row['username'];	
			...
			$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan, a.nama_pengurus, a.salesman,
										  a.sesuai_ktp, DATE_FORMAT(b.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas, DATE_FORMAT(b.tgl_kunjungan_berikut, '%d %M %Y') AS tgl_kunjungan_berikut, k.status_prospek,
										  d.from_salesman, d.to_salesman, e.nama_database 
								   FROM tb_pelanggan a
    							   ...
    							   LEFT JOIN ms_status_prospek_suzuki k ON k.id_status_prospek = j.id_status_prospek
								   LEFT JOIN tb_import d ON d.data_import = a.id_pelanggan
								   LEFT JOIN tb_database e ON e.id_database = a.id_database
								   LEFT JOIN ms_kecamatan f ON f.id_kecamatan = a.id_kecamatan
								   LEFT JOIN ms_kelurahan g ON g.id_kelurahan = a.id_kelurahan

								   WHERE a.nama_perush LIKE '%$nama_perush%' 
								   AND a.salesman = '$sales'
								   AND a.deleted_flag = '0'
								   AND (
								   		 	d.to_salesman = '$sales'
										AND d.deleted_flag = '0'
										OR  d.deleted_flag IS NULL
									   	)
								   
								   AND (
		 								   f.nama_kecamatan LIKE '%$alamat%'
										OR g.nama_kelurahan LIKE '%$alamat%'
										OR a.alamat_kantor LIKE '%$alamat%'
									   )

								   ORDER BY a.id_pelanggan DESC ") or die (mysql_error());

			} elseif($_SESSION[sublevel] == 'part'){
			$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan, a.nama_pengurus, a.jabatan, a.salespart,
										  a.sesuai_ktp,  DATE_FORMAT(b.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas, DATE_FORMAT(b.tgl_kunjungan_berikut, '%d %M %Y') AS tgl_kunjungan_berikut, k.status_prospek,
										  d.from_salesman, d.to_salesman, e.nama_database 
								   FROM tb_pelanggan a
								   LEFT JOIN v_part_latest_activity_id i ON i.id_pelanggan = a.id_pelanggan
								   LEFT JOIN tb_aktivitas_harian_part b ON b.id_aktivitas_harian = i.max_aktivitas
								   LEFT JOIN tb_import d ON d.data_import = a.id_pelanggan
								   LEFT JOIN tb_database e ON e.id_database = a.id_database
								   LEFT JOIN ms_kecamatan f ON f.id_kecamatan = a.id_kecamatan
								   LEFT JOIN ms_kelurahan g ON g.id_kelurahan = a.id_kelurahan
			...
			$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan, a.nama_pengurus, a.salesserv,
										  a.sesuai_ktp,  DATE_FORMAT(b.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas, DATE_FORMAT(b.tgl_kunjungan_berikut, '%d %M %Y') AS tgl_kunjungan_berikut, 
										  d.from_salesman, d.to_salesman, e.nama_database 
								   FROM tb_pelanggan a
								   LEFT JOIN v_serv_latest_activity_id i ON i.id_pelanggan = a.id_pelanggan
								   LEFT JOIN tb_aktivitas_harian_serv b ON b.id_aktivitas_harian = i.max_aktivitas
								   LEFT JOIN tb_import d ON d.data_import = a.id_pelanggan
								   LEFT JOIN tb_database e ON e.id_database = a.id_database
								   LEFT JOIN ms_kecamatan f ON f.id_kecamatan = a.id_kecamatan
								   LEFT JOIN ms_kelurahan g ON g.id_kelurahan = a.id_kelurahan
			...
			if($_SESSION[sublevel] == 'unit'){
			$result = mysql_query("SELECT a.id_pelanggan, b.nama_perush, b.nama_pengurus, 
										  DATE_FORMAT(a.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas,
										  DATE_FORMAT(a.tgl_kunjungan_berikut , '%d %M %Y') AS tgl_kunjungan_berikut, 
										  d.status_prospek, b.salesman, e.from_salesman
								   FROM	v_unit_latest_activity a
									
								   JOIN (SELECT	id_pelanggan, nama_perush,	nama_pengurus, salesman
								   		  FROM	tb_pelanggan
								   		  WHERE	salesman LIKE '%$sales%' AND nama_perush LIKE '%$nama_perush%' AND alamat_rumah LIKE '%$alamat%'
								   		  AND deleted_flag = 0 ) b ON b.id_pelanggan = a.id_pelanggan

								   LEFT JOIN (SELECT id_pelanggan,max_prospek,id_status_prospek 
												FROM v_unit_latest_prospek_id) c ON c.id_pelanggan=a.id_pelanggan
								   
								   LEFT JOIN ms_status_prospek_suzuki d ON d.id_status_prospek = c.id_status_prospek
								   
								   LEFT JOIN (SELECT from_salesman, to_salesman, data_import FROM tb_import  WHERE to_salesman = '%$sales%') e ON e.data_import = a.id_pelanggan
								   
								   WHERE a.tgl_aktivitas BETWEEN '$from' AND '$to'
								   
								   ORDER BY a.tgl_aktivitas DESC") or die (mysql_error());

			} elseif($_SESSION[sublevel] == 'part'){
			$result = mysql_query("SELECT a.id_pelanggan, b.nama_perush, b.nama_pengurus, 
										  DATE_FORMAT(a.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas,
										  DATE_FORMAT(a.tgl_kunjungan_berikut , '%d %M %Y') AS tgl_kunjungan_berikut, 
										  d.status_prospek, b.salespart, e.from_salesman
								   FROM	v_unit_latest_activity a
									
								   JOIN (SELECT	id_pelanggan, nama_perush,	nama_pengurus, salespart
								   		  FROM	tb_pelanggan
								   		  WHERE	salespart = '$sales' AND nama_perush LIKE '%$nama_perush%' AND alamat_rumah LIKE '%$alamat%'
								   		  AND deleted_flag = 0 ) b ON b.id_pelanggan = a.id_pelanggan
			...
			$result = mysql_query("SELECT a.id_pelanggan, b.nama_perush, b.nama_pengurus, 
										  DATE_FORMAT(a.tgl_aktivitas, '%d %M %Y') AS tgl_aktivitas,
										  DATE_FORMAT(a.tgl_kunjungan_berikut , '%d %M %Y') AS tgl_kunjungan_berikut, 
										  d.status_prospek, b.salesserv, e.from_salesman
								   FROM	v_unit_latest_activity a
									
								   JOIN (SELECT	id_pelanggan, nama_perush,	nama_pengurus, salesserv
								   		  FROM	tb_pelanggan
								   		  WHERE	salesmanserv = '$sales' AND nama_perush LIKE '%$nama_perush%' AND alamat_rumah LIKE '%$alamat%'
								   		  AND deleted_flag = 0 ) b ON b.id_pelanggan = a.id_pelanggan
					...
					<div class="portlet box purple">
						...
						<div class="portlet-body">
							...
							<tr>
								...
								<th>Pengurus</th>
								<?php if($_SESSION[sublevel] == 'part'){ ?>
								<th>Jabatan</th>
								<?php } ?>
								<th>Tgl Aktivitas</th>
								<th>Tgl Knjngn Brkt</th>
								<?php if($_SESSION[sublevel] == 'unit'){ ?>
								<th>Status</th>
								<?php } ?>
							...
							<tr>
								...
								<td><?php echo $row['tgl_kunjungan_berikut'];?></td>
								<?php if($_SESSION[sublevel] == 'unit'){ ?>
			...
			<div class="portlet box blue">
				<div class="portlet-title">
					...
					$tambah = mysql_query("INSERT INTO tb_pelanggan(
												 ...
												 nama_perush,alamat_kantor,telp_kantor, fax_kantor,email_kantor,nama_pengurus,
											...
											VALUES('','$kode_pelanggan','$id_asal_prospek','$id_database','$id_bidang_usaha','$nama_perush','$alamat_kantor','$telp_kantor','$fax_kantor','$email_kantor','$nama_pengurus','$jabatan','$no_hp','$email','$no_ktp','$tmpt_lahir','$tgl_lahir','$jns_kelamin','$agama' ,'$gol_darah' ,'$status_perkawinan','$id_pekerjaan','$kewarganegaraan','$id_prov' ,'$id_kota','$id_kecamatan' ,'$id_kelurahan' ,'$alamat_rumah','$rt','$rw','$kodepos','$telp_rumah','$hobi1','$hobi2','$hobi3','$hobi4','$salesman','$salesman',NOW())") 
											OR DIE(mysql_error());
					...
					$tambah = mysql_query("INSERT INTO tb_pelanggan (
												 ...
												 fax_kantor,email_kantor,nama_pengurus,
												 jabatan,no_hp,email,no_ktp,tmpt_lahir,tgl_lahir,jns_kelamin,agama,gol_darah,
												 status_perkawinan,id_pekerjaan,kewarganegaraan,id_prov,id_kota,id_kecamatan,
												 id_kelurahan,alamat_rumah,rt,rw,kodepos,telp_rumah,hobi1,hobi2,hobi3,hobi4,
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<select class="form-control select2me" name="id_asal_prospek">
												<option selected="selected">- Pilih Asal Prospek -</option>
													<?php
										...
										<div class="form-group">
											...
											<select class="form-control select2me " name="id_database">
												<option selected="selected">- Pilih Kode Database -</option>
													<?php
														$sql=mysql_query("SELECT * FROM tb_database WHERE deleted_flag=0 AND divisi = 'suzuki'");
															while($row=mysql_fetch_array($sql))
																	{
														$id = $row['id_database'];	
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<select class="form-control " required="required" name="id_bidang_usaha">
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<input type="text" class="form-control" autocomplete="off" name="nama_pengurus" placeholder="Nama Pemilik/Pengurus" >
											<span class="help-block">*Wajib diisi </span>
										</div>
									</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<input type="text" class="form-control" autocomplete="off" name="jabatan" placeholder="Jabatan" >
											<span class="help-block">*Wajib diisi </span>
										</div>
									</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<input type="text" class="form-control" autocomplete="off" name="no_hp" placeholder="No. Handphone" >
											<span class="help-block">*Wajib diisi </span>
										</div>
									</div>
						...
						<div class="portlet-body">
							...
							<?php
								$result = mysql_query("SELECT a.id_pelanggan, a.nama_perush, a.id_pelanggan, a.nama_pengurus, a.salesman,
								   ...
								   AND a.deleted_flag = '0'
								   AND (
								   		 	d.to_salesman = '$_SESSION[username]'
										AND d.deleted_flag = '0'
										OR  d.deleted_flag IS NULL
									   	)
							...
							<tr>
								<td>#</td>
								<td><?php echo $row['id_pelanggan'];?></td>
								<td><a href="?module=data_customer&act=detail&id_pelanggan=<?php echo $row['id_pelanggan'];?>" target="_blank">			<?php echo $row['nama_perush'];?>
									</a>
								</td>
								<td><?php echo $row['nama_pengurus'];?></td>
								<?php if($_SESSION[sublevel] == 'part'){ ?>
			...
			<div class="portlet box purple">
				...
				$result = mysql_query("SELECT * FROM tb_pelanggan LEFT JOIN tb_kuisioner ON tb_kuisioner.id_pelanggan = tb_pelanggan.id_pelanggan WHERE tb_pelanggan.id_pelanggan='$id_pelanggan'");
						...
						<div class="form-body">
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Nama Perusahaan/Pribadi</label>
											<input type="text" class="form-control" autocomplete="off" name="nama_perush" value="<?php echo $array['nama_perush']; ?>" disabled>
										</div>
									</div>
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Alamat Kantor</label>
											<textarea class="form-control" rows="3" name="alamat_kantor" disabled><?php echo $array['alamat_kantor']; ?></textarea>
										</div>
									</div>
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Nama Pemilik/Pengurus</label>
											<input type="text" class="form-control" autocomplete="off" name="nama_pengurus" value="<?php echo $array['nama_pengurus']; ?>" disabled>
										</div>
									</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Jabatan</label>
											<input type="text" class="form-control" autocomplete="off" name="jabatan" value="<?php echo $array['jabatan']; ?>" disabled>
										</div>
									</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">No. Hp</label>
											<input type="text" class="form-control" autocomplete="off" name="no_hp" value="<?php echo $array['no_hp']; ?>" disabled>
										</div>
									</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Email</label>
											<input type="text" class="form-control" autocomplete="off" name="email" value="<?php echo $array['email']; ?>" disabled>
										</div>
									</div>
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Alamat Rumah</label>
											<textarea class="form-control" rows="3" autocomplete="off" name="alamat_rumah" disabled><?php echo $array['alamat_rumah']; ?></textarea>
										</div>
									</div>
									...
									<div class="col-md-1">
										<div class="form-group">
											...
											<input type="checkbox" name="sesuai_ktp" value="ya" <?php if ($array['sesuai_ktp'] == 'ya' ) { echo "checked" ; } ?> class="make-switch switch-large" data-label-icon="fa fa-fullscreen" data-on-text="<i class='fa fa-check'></i>" data-off-text="<i class='fa fa-times'></i>" disabled> 
										</div>
									</div>
			...
			<div class="portlet box purple">
				<div class="portlet-title">
					...
					<div class="actions">
						<a href="?module=aktivitas&act=add&aktivitas=add_telp&id_pelanggan=<?php echo $id_pelanggan ?>" class="btn btn-default btn-sm">
						<i class="fa fa-plus"></i> Tambah Aktivitas Harian </a>						
				  	</div>
				</div>
				<div class="portlet-body">
						<table class="table table-striped table-bordered table-hover" id="aktivitas">
							<thead>
								<tr>
									<th>#</th>
									<th>Tgl Aktivitas</th>
									<th>Aktivitas <?php echo $tab_small ; ?></th>
						...
						<table class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th>Action Prospek <?php echo $tab_small ; ?></th>
								</tr>
							</thead>
							<tbody>
								<?php
									$no=1;
                           			$result = mysql_query("SELECT a.id_prospek,	a.id_pelanggan,
                           									...
                           									FROM (
																	...
																	FROM
																		tb_prospek a
																	GROUP by no_prospek,id_prospek 
																) a
															...
															WHERE a.id_pelanggan = $id_pelanggan AND a.deleted_flag = 0 ORDER BY id_prospek DESC");
												while ($row = mysql_fetch_array($result)) {
							 	?>
								<tr>
									...
									<td><?php echo str_replace($en, $idn, $row['tgl_estimasi']); ?></td>
									...
									<td align="center">
										 ...
										 <?php if($row['id_status_prospek'] == '1' OR $row['id_status_prospek'] == '2') { ?>
										 ...
										 <i class="fa fa-edit"></i></a>
										 <?php } elseif ($row['id_status_prospek'] == '3') { ?>
										 ...
										 <a href="modul/laporan/cetak_kdp.php?id_prospek=<?php echo $row[id_prospek] ;?>" target="_blank" class="btn btn-xs red tooltips" data-original-title="Cetak KDP" >
										 	<i class="fa fa-fax"></i>
										 </a>
									</td>
								</tr>
								...
								<tr>
									<td colspan="2"> <strong>Keterangan Join Visit</strong> </td>
									<td colspan="13"> </td>
								</tr>
				...
				<?php
					...
					$post = (!empty($_POST)) ? true : false;
						...
						if(empty($error)){
							...
							$nama_perush_asal = $_POST['nama_perush_asal'];
							$nama_pengurus_asal = $_POST['nama_pengurus_asal'];
							$no_hp_asal = $_POST['no_hp_asal'];
							$alamat_rumah_asal = $_POST['alamat_rumah_asal'];
					...
					$edit = mysql_query("UPDATE tb_pelanggan 
										   SET kode_pelanggan = '$kode_pelanggan',id_asal_prospek ='$id_asal_prospek',
										   	   ...
										   	   nama_perush='$nama_perush',alamat_kantor='$alamat_kantor',
										   	   telp_kantor='$telp_kantor',fax_kantor='$fax_kantor',email_kantor='$email_kantor',
						...
						<div class="form-body">
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Bidang Usaha</label>
											<select class="form-control" name="id_bidang_usaha">
											<option>- Pilih Bidang Usaha -</option>
													<?php
														$sql=mysql_query("SELECT * FROM ms_bidang_usaha");
															while($row=mysql_fetch_array($sql))
