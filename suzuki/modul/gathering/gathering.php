		...
		<div class="page-content">
				...
				<div class="col-md-12">
					...
					<div class="portlet box purple">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-reorder"></i>Tabel Data Gathering
							</div>
							<div class="actions">
								<a href="?module=gathering&act=tambah" class="btn btn-default btn-sm">
								<i class="fa fa-plus"></i> Tambah Gathering </a>
						...
						<div class="portlet-body">
							...
							<thead>
							<tr>
								<th width="30%">Nama Gathering</th>
								<th width="15%">Tanggal</th>
								<th width="25%">Lokasi</th>
								<th width="15%">Undangan</th>
								<?php if ($_SESSION[leve] == 'admin') { ?>
							...
							<?php
                           	$result = mysql_query("SELECT * FROM tb_gathering WHERE deleted_flag = 0 ORDER BY id_gathering ASC");
							while ($row = mysql_fetch_array($result)) {
							 ?>

							<tr>
								<td><?php echo $row['nama_gathering'] ?></td>
								<td><?php echo $row['tgl_gathering']?></td>
								<td><?php echo $row['lokasi']?></td>
								<td>
									<a href="?module=gathering&act=undangan&id_gathering=<?php echo $row['id_gathering'] ; ?>"
											class="btn default btn-xs purple">
										<i class="fa fa-edit"></i> Klik Untuk Melihat Undangan </a>
								</td>
								<?php if ($_SESSION[leve] == 'admin') { ?>
								<td>
									<a href="?module=gathering&act=delete&id_gathering=<?php echo $row['id_gathering'] ; ?>" 	class="btn btn-xs red tooltips" data-original-title="Hapus Data">
										 <i class="fa fa-times"></i>
									</a>

									<a href="?module=gathering&act=edit&id_gathering=<?php echo $row['id_gathering'] ; ?>" 	class="btn btn-xs green tooltips" data-original-title="Edit Data">
                  ...
                  <div class="portlet-body">
					 ...
					 <form method="post" action="">
                        <div class="row">
                            ...
                            <div class="col-md-1">
						 	    <label class="control-label"> &nbsp</label>
								<a href="?module=gathering" type="button" class="btn default">Cancel</button></a>
                            </div>
							...
							<div class="col-md-8">
								...
								<select class="form-control form-control-inline select2me" name="sales">
										<option selected="selected">- Pilih Salesman -</option>
											<?php
 											$sql= mysql_query("SELECT * FROM v_users WHERE jabatan IN ('sales','supervisor') AND divisi = 'hino' AND department = 'unit' ORDER BY salesman ASC");
 											while($row=mysql_fetch_array($sql))
																	{
											$id = $row['salesman'];	
  												
											echo '<option value="'.$id.'">'.$row['salesman'].'</option>';
													}
											?>
								</select>
							</div>
			...
			if ($_SESSION[level] == 'admin') {
				...
				$result = mysql_query("SELECT a.id_pelanggan,a.nama_perush,a.nama_pengurus, b.segmen, a.salesman,
                           			   ...
                           			   LEFT JOIN v_latest_prospek_id d ON d.id_pelanggan = a.id_pelanggan
