<div class="page-content-wrapper">
		<div class="page-content">
		    ...
		    $table1 = "SELECT   tb_pelanggan.`id_pelanggan`, 
		            			tb_pelanggan.`salesman`, 
		            			tb_pelanggan.`nama_perush`, 
		                        v_data_users.`supervisor`, v_data_users.`grade`, v_data_users.`poin`, v_data_users.`nik`, v_data_users.`divisi`, v_data_users.`department`,
		            			COUNT(mi.visit) AS 'visit',
		            			COUNT(mi.telp) AS 'telp',
		            			COUNT(mi.survey) AS 'survey'
		            		   FROM tb_pelanggan
		            		   LEFT JOIN ($v_act) mi 
		            		     ON mi.id_pelanggan = tb_pelanggan.`id_pelanggan`
							   JOIN v_data_users ON v_data_users.`salesman` = tb_pelanggan.`salesman`
		                       WHERE v_data_users.`jabatan` = 'sales'
		            	       GROUP BY tb_pelanggan.`id_pelanggan`
		            	       ORDER BY tb_pelanggan.`salesman`, tb_pelanggan.`nama_perush`";

		    $table2 = "SELECT v_unit_prospek_rowtocolumn.nama_perush, tb_do.tgl_do, v_unit_prospek_rowtocolumn.`id_pelanggan`, 
		    				  v_unit_prospek_rowtocolumn.`do` 
		            		  FROM tb_prospek 
		            		  JOIN v_unit_prospek_rowtocolumn ON v_unit_prospek_rowtocolumn.`id_pelanggan` = tb_prospek.`id_pelanggan` 
		            		  JOIN tb_do ON tb_do.`id_pelanggan` = tb_prospek.`id_pelanggan` 
		            		  WHERE tb_do.tgl_do BETWEEN '$from' AND '$to'
		            		        
		            		  AND id_status_prospek = '5' 
		    ...
		    $table4 = "SELECT tb_pelanggan.id_pelanggan AS id_pelanggan, tb_pelanggan.salesman AS salesman,
		    				  ...
		    				  tb_prospek.jml_kendaraan  AS jml_kendaraan,
		        		      tb_prospek.created_date AS 'prospek_baru'
		        	          FROM tb_pelanggan
			        		  ...
			        		  AND tb_prospek.`created_date` BETWEEN '$from' AND '$to'
			        		  AND tb_pelanggan.deleted_flag = 0
			        		  AND tb_prospek.deleted_flag = 0";
		    $tableProspek = "SELECT v_unit_prospek_rowtocolumn.`salesman`,
    		...
    		$fieldPolreg = "SELECT 	tb_pelanggan.`salesman`, tb_aktivitas_harian_unit.`id_aktivitas_harian`, 
		    ...
		    $query = "SELECT table1.supervisor, table1.salesman, table1.grade, table1.poin, tableProspek.prospekTouch,
		    				 tableProspek.prospekNego, tableProspek.prospekSpk, tableProspek.prospekDo, 
		    				 SUM(visit) AS 'jml_visit', SUM(telp) AS 'jml_telp', SUM(survey) AS 'jml_survey', 
		    				 tablePolreg.jml_polreg, COUNT(DO) AS 'jml_do', table3.new_customer
		                
		            		FROM ($table1) AS table1
		            		LEFT JOIN ($table2) AS table2 ON table2.id_pelanggan = table1.id_pelanggan
		            		LEFT JOIN ($table3) table3 ON table3.salesman = table1.salesman
		                 	LEFT JOIN ($tableProspek) tableProspek ON tableProspek.salesman = table1.salesman
		                 	LEFT JOIN ($fieldPolreg) tablePolreg ON tablePolreg.salesman =  table1.salesman
		                 	WHERE table1.divisi = '$_SESSION[divisi]' AND table1.department = 'unit'
		             		GROUP BY table1.salesman
		             		ORDER BY table1.supervisor,table1.salesman  ASC";
